...
try {
    ...
    die("Database error while loading gateways.");
}
...
<head><?= csrf_meta() ?>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .card-title { padding: 24px; border-bottom: 1px solid var(--border); font-family: var(--font-head); font-size: 18px; font-weight: 700; display: flex; align-items: center; justify-content: space-between; gap: 10px; background: rgba(0,0,0,0.1); }
    ...
    .badge-enabled { background: rgba(34,211,238,0.1); color: var(--accent-green); border: 1px solid rgba(34,211,238,0.2); }
    .badge-disabled { background: var(--surface2); color: var(--text-muted); border: 1px solid var(--border); }
  </style>
</head>
<body>
...
<main>
  ...
  <header>
    <div class="header-title">Payment Gateways</div>
    ...
    <div class="card">
        <div class="card-title">
            <div style="display: flex; align-items: center; gap: 10px;">
                <i class="fa-solid fa-building-columns" style="color: var(--accent);"></i> Active Gateways
            </div>
        </div>
        <table>
            <thead>
                <tr>
                    <th width="30%">Gateway Name</th>
            ...
            <tbody>
                <?php if(empty($gateways)): ?>
                    <tr><td colspan="3" style="text-align: center; padding: 48px; color: var(--text-dim);">No gateways configured.</td></tr>
                <?php else: foreach($gateways as $g): ?>
                    <tr>
                        <td style="font-weight: 600; color: var(--text);"><?= htmlspecialchars($g['name']) ?></td>
                        <td>
                            <span class="badge <?= $g['status'] == 'enabled' ? 'badge-enabled' : 'badge-disabled' ?>">
                                <?= htmlspecialchars($g['status']) ?>
                            </span>
                        </td>
                        <td style="color: var(--text-dim); font-size: 13px; font-family: var(--font-mono);">
                            <?= date('M j, Y - g:i A', strtotime($g['updated_at'])) ?>
                        </td>
                    </tr>
